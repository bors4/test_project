...
class PageObjects {
   ...
   */
  /**
   * @todo описать метод таким образом, чтобы он принимал любое кол-во элементов
   * и возвращал elementText, если все элементы содержат один и тот же текст.
   */
  async getElementText(elementName, pageName) {
    const element = await this.getElementsByName(elementName, pageName);
    const elementText = await element.map((el) => el.getText());
